...
func init() {
	...
	runCmd.Flags().String("enable-inboud-ipv6", "false", "whether inbound ipv6 connection should be managed by the mesh (currently is must be set to false)")
	runCmd.Flags().String("envoy-port", "15000", "Specify the envoy port to which redirect all TCP traffic. This is a cluster-wide setting, you can override it by adding this annotation to your pods 'pod-network-controller.istio.io/envoy-port'")
	runCmd.Flags().String("istio-inbound-interception-mode", "REDIRECT", "The mode used to redirect inbound connections to Envoy, either REDIRECT or TPROXY. his is a cluster-wide setting, you can override it by adding this annotation to your pods 'pod-network-controller.istio.io/intercept-mode'")
	runCmd.Flags().String("istio-include-inbound-ports", "''", "comma-separated list of ports that will be redirected to Envoy. This list will be added to the ports desumed from the service controlling the pod. The wildcard character \"*\" can be used to configure redirection for all ports. This is a cluster-wide settings, you can override it by adding this annotation to your pods 'pod-network-controller.istio.io/include-inbound-ports'")
	runCmd.Flags().String("istio-exclude-inbound-ports", "''", "Comma separated list of inbound ports to be excluded from redirection to Envoy (optional). Only applies when all inbound traffic (i.e. \"*\") is being redirected. This is a cluster-wide settings, you can override it by adding this annotation to your pods 'pod-network-controller.istio.io/exclude-inbound-ports'")
	runCmd.Flags().String("istio-include-outbound-cidrs", "*", "Comma separated list of IP ranges in CIDR form to redirect to envoy (optional). The wildcardcharacter \"*\" can be used to redirect all outbound traffic. An empty list will disable all outbound redirection. This is a cluster-wide settings, you can override it by adding this annotation to your pods 'pod-network-controller.istio.io/include-outbound-cidrs'")
	runCmd.Flags().String("istio-exclude-outbound-cidrs", "''", "Comma separated list of IP ranges in CIDR form to be excluded from redirection. Only applies when all outbound traffic (i.e. \"*\") is being redirected. This is a cluster-wide settings, you can override it by adding this annotation to your pods 'pod-network-controller.istio.io/exclude-outbound-cidrs'")
	runCmd.Flags().String("envoy-userid", "1337", "UID used by the envoy-proxy container. This is a cluster-wide settings, you can override it by adding this annotation to your pods 'pod-network-controller.istio.io/envoy-userid'")
	runCmd.Flags().String("envoy-groupid", "1337", "GID used by the envoy-proxy container. This is a cluster-wide settings, you can override it by adding this annotation to your pods 'pod-network-controller.istio.io/envoy-groupid'")
	...
	viper.BindPFlag("enable-inboud-ipv6", runCmd.Flags().Lookup("enable-inboud-ipv6"))
